...
namespace WMG.DVDCentral.PL.Test
{
    ...
    {
        const string connstrlocal = "Data Source=(localdb)\\MSSQLLocalDB;Initial Catalog=WMG.DVDCentral.DB;Integrated Security=True";
        const string tableStructure = "DVDCentralV1";
                                          ...
                                          ref string errmessage)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (columnName != textInfo.ToTitleCase(columnName.ToLower()))
                    {
                        // Check the Title case version
                        string titleCase = textInfo.ToTitleCase(columnName.ToLower());
                        if (CheckColumnDefinition(tableType,
                                                   titleCase,
                                                   dataType,
                                                   ref message,
                                                   ref errmessage))
        ...
        public void CheckTableStructure()
        {
            ...
            foreach (Structure structure1 in structures)
            {
                ...
                if (tableType == null)
                {
                    // Student is missing a class in the PL.
                    results = false;
                    message += $"Failed: {tableTypeName} class is missing in the PL.\n";
                    errMessage += $"Failed: {tableTypeName} class is missing in the PL.\n";
                }
                else
                {
                    ...
                    results = CheckCounts(tableType, ref message, ref errMessage) ? results : false;
                }
                message += "-----------------------------------\n";
            }
        ...
        private static List<Structure> ReadStructures()
        {
            ...
            try
            {
                ...
                HttpResponseMessage response = client.GetAsync($"TableStructure/{tableStructure}").Result;
